...
export async function FindPackages(): Promise<Packages> {
    return new Promise(async (resolve, reject) => {
        let packages: Packages = {
            head: "",
            body: ""
        }
        let file_exists = await fse.pathExists(`${base}/src/packages.feno`)
        if (file_exists) {
            let packages_data = await fse.readFile(`${base}/src/packages.feno`, "utf8")
...
export async function SetPackages(html: string): Promise<string> {
    return new Promise(async (resolve, reject) => {

        let packages = await FindPackages()
        html = html.replace(/<head>([\s\S]*?)<\/head>/, `<head>$1\n${packages.head}</head>`)
        html = html.replace(/<body>([\s\S]*?)<\/body>/, `<body>$1\n${packages.body}</body>`)

...
module.exports = { $Watch: () => {
    ...
    fse.readdir(`${base}/src/components`,(err:string,files:string[]) => {
        if (err) console.error(err);
        if (files.length) {
            files.forEach(file => {
                var basename:string = path.basename(file, path.extname(file)); // nombre del componente
                /** Leer cada componente */
                fse.readFile(`${base}/src/components/${file}`,'utf8',(err:string,data:string) => {
                    if (err) console.error(err);
                    fse.readdir(`${base}/src/scripts`,(err:string,sfiles:string[]) => {
                        if (err) console.error(err);
                        sfiles.forEach(sfile => {
                            const basename_script:string = path.basename(sfile, path.extname(sfile));
                            fse.readFile(
                                /*`./public/${basename_script}.html`,*/
                                path.join(path.dirname(require.resolve('graphtml')),`/public/${basename_script}.html`),
                                'utf8',(err:string,sdata:string) => {
                                    ...
                                    if (word == basename) {
                                        ...
                                        fse.writeFile(
                                            ...
                                            lines.forEach(async (line) => {
                                                ...
                                                fse.readdir(`${base}/src/components`,(err,files) => {
                                                    files.forEach(file => {
                                                        var basename:string = path.basename(file, path.extname(file)); // nombre del componente
                                                        fse.readFile(`${base}/src/components/${file}`,'utf8',async (err,data) => {
                                                            if (word == basename) {
                                                                let tag:string = `call(${word})`;
                                                                // Eliminar el llamado en Nue e insertar el contenido del componente
                                                                let html:string = await Core.Process(data,'component');
